...
//게시판
$write_table = $g5['chadeal_application_table'];
...
function chgState($wr_id, $CD_BOARDID)
{
    ...
    if (isset($_POST['NM_STATE'][$CD_BOARDID])) {
        $ca_name = addslashes(clean_xss_tags($_POST['NM_STATE'][$CD_BOARDID]));
    }
...
function chgMemo($wr_id, $CD_BOARDID)
{
    ...
    if (isset($_POST['NM_MEMO'][$CD_BOARDID])) {
        $wr_seo_title = substr(trim($_POST['NM_MEMO'][$CD_BOARDID]), 0, 65536);
        $wr_seo_title = preg_replace("#[\\\]+$#", "", $wr_seo_title);
    }

    $sql = " update {$write_table}
                set wr_seo_title = '{$wr_seo_title}'
              where wr_id = '{$wr_id}' ";
    sql_query($sql);
