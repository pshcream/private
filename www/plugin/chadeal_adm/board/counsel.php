...
if (isset($sSTATUS) && strlen($sSTATUS)) {
    $sql_search .= " and ca_name='" . str_replace('상담신청', '', $sSTATUS) . "' ";
}
...
if (isset($sKEYWORD) && strlen($sKEYWORD)) {
    switch ($sGUBUN) {
        case "usernm":
            $sql_search .= " and wr_name like '%" . trim($sKEYWORD) . "%' ";
            break;
        case "phone":
            $sql_search .= " and wr_content like '%" . trim($sKEYWORD) . "%' ";
            break;
        case "memo":
            $sql_search .= " and wr_seo_title like '%" . trim($sKEYWORD) . "%' ";
            break;
    }
}
...
$total_res = sql_fetch("select count(*) as cnt from {$g5['chadeal_application_table']} where 1 $sql_group $sql_search");
...
//출고
$res = sql_fetch("select count(*) as cnt from {$g5['chadeal_application_table']} where lo_cus_name!='' $sql_group $sql_search");
$cnt1 = $res['cnt'];

$res = sql_fetch("select count(*) as cnt from {$g5['chadeal_application_table']} where lo_cus_name='' $sql_group $sql_search");
$nonpre_count = $res['cnt'];

$res = sql_fetch("select count(*) as cnt from {$g5['chadeal_application_table']} where lo_expose='N' $sql_group $sql_search");
$hid_count = $res['cnt'];
...
<body>
                                    ...
                                    <div class="table-responsive">
                                        ...
                                        <table class="table table-hover table-striped">
                                            ...
                                            <tbody>
                                                ...
                                                $sql = "select * from {$g5['chadeal_application_table']} where 1 $sql_group $sql_search order by wr_id desc limit $page_start, $page_size";
                                                $qry = sql_query($sql);
                                                ...
                                                for ($i = 0; $res = sql_fetch_array($qry); $i++) {
                                                    ...
                                                    $memo = get_text($res['wr_seo_title'], 0);
                                                    $estSheet = "";
                                                    $estRes = "";
                                                    $cartype = $res['wr_4'];
                                                    if ($res['wr_link1'] && $res['wr_link2']) {
                                                        $estRes = getEstDetail($res['wr_link1'], $res['wr_link2'], $res['wr_10'], $res['wr_1'], "");
                                                        $estSheet = "<a href='/sheet/{$res['wr_link1']}/{$res['wr_link2']}' target='_blank'>견적서확인</a>";
                                                    } else if ($res['wr_9'] == "선출고") {
                                                        $estRes = getEstDetail("", "", $res['wr_10'], $res['wr_1'], $res['wr_id']);
                                                    } else {
                                                        $estRes = getEstDetail("", "", $res['wr_10'], $res['wr_1'], "");
                                                    }
                                                ?>
                                                    <tr>
                                                        ...
                                                        <td class="text-center"><?php echo $res['wr_9']; ?></td>
                                                        <td class="text-center"><?php echo $res['wr_3']; ?></td>
                                                        <td class="text-center"><?php echo $estSheet; ?></td>
                                                        <td class="text-center"><?php echo get_text($res['wr_subject']); ?></td>
                                                        <td class="text-center"><?php echo $res['wr_name']; ?></td>
                                                        <td class="text-center"><?php echo hyphen_hp_number(get_text($res['wr_content'])); ?></td>
                                                        <td class="no-padding">
                                                        ...
                                                        <td class="text-center ">
                                                            <textarea name="NM_MEMO[<?php echo $i ?>]" class="form-control no-padding" style="min-width:200px; width: 100%; height: 120px; resize: none;"><?php echo $memo; ?></textarea>
                                                        </td>
                                                        <td class="text-center ">
                                                            <select class="form-control no-padding" name="NM_STATE[<?php echo $i ?>]" onChange="fn_counsel('state','<?php echo $i ?>');" style="margin-bottom: 5px;">
                                                                <option value="" <?php echo ($res['ca_name'] == "") ? "selected" : ""; ?>>상담신청</option>
                                                                <option value="상담취소" <?php echo ($res['ca_name'] == "상담취소") ? "selected" : ""; ?>>상담취소</option>
                                                                <option value="상담중" <?php echo ($res['ca_name'] == "상담중") ? "selected" : ""; ?>>상담중</option>
                                                                <option value="상담완료" <?php echo ($res['ca_name'] == "상담완료") ? "selected" : ""; ?>>상담완료</option>
                                                                <option value="계약완료" <?php echo ($res['ca_name'] == "계약완료") ? "selected" : ""; ?>>계약완료</option>
                                                                <option value="출고완료" <?php echo ($res['ca_name'] == "출고완료") ? "selected" : ""; ?>>출고완료</option>
                                                            </select>
    ...
    <script type="text/javascript">
        ...
        function fn_regist() {
            document.location.href = "/chadeal_adm/board/counsel_write.php";
        }
    </script>
