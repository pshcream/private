...
if (!isset($_REQUEST['g_mb_id']) || !$_REQUEST['g_mb_id']) {
    die(0);
} else {
    $mb_id = addslashes(clean_xss_tags(trim($_REQUEST['g_mb_id'])));
}
...
if (!isset($_REQUEST['g_sheet_idx']) || !$_REQUEST['g_sheet_idx']) {
    die(0);
} else {
    $sheet_idx = addslashes(clean_xss_tags(trim($_REQUEST['g_sheet_idx'])));
}
...
$idx = "g" . $est_idx . strtotime(date("Y-m-d h:i:s"));
...
$sql = " insert into {$g5['chadeal_garagelist_table']}
set idx = '$idx',            
    mb_id = '$mb_id',
    est_idx = '$est_idx',            
    sheet_idx = '$sheet_idx', 
