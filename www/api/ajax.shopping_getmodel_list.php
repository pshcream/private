...
$list = array();
$qry = sql_query("SELECT * FROM (SELECT * FROM (SELECT a.BRN_C_NO, a.BRN_C_NM, a.MODL_C_NO, a.MODL_C_NM, a.DTL_TRIM_C_NO, a.NATION, a.CLASSIFYCODE, b.R48M2, b.L48M2 FROM {$g5['shopping_cardb_table']} a LEFT JOIN {$g5['shopping_epdata_table']} b ON a.DTL_TRIM_C_NO = b.DTL_TRIM_C_NO WHERE (R48M2 NOT IN(0)) AND (L48M2 NOT IN(0)) $sql_where) AS c GROUP BY MODL_C_NO) AS d LEFT JOIN {$g5['shopping_count_table']} AS e ON d.MODL_C_NO = e.MODL_C_NO ORDER BY EP_COUNT DESC, R48M2 DESC");
while ($res = sql_fetch_array($qry)) array_push($list, $res);

$modelList = array();
$modelQry = sql_query("SELECT * FROM (SELECT a.MODL_C_NO, a.MODL_C_NM FROM {$g5['shopping_cardb_table']} a LEFT JOIN {$g5['shopping_epdata_table']} b ON a.DTL_TRIM_C_NO = b.DTL_TRIM_C_NO WHERE (R48M2 NOT IN(0)) AND (L48M2 NOT IN(0)) $sql_where) AS c GROUP BY MODL_C_NO");
    ...
    <li class="car-item">
        ...
        <div class="tbox">
            ...
            <div class="car-item-btnbox">
                <button type="button" class="cib-btn" id="cib-btn-estimate" data-brandnm="' . $brandNm . '" data-modelnm="' . $modelNm . '" data-id="' . $trimNo . '">견적받기</button>
