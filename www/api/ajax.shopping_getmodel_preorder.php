...
$modelList = array();
if ($nation == "KR") {
    $modelQry = sql_query("SELECT DISTINCT brnNo, brnNm, modlNo, modlNm FROM (SELECT DISTINCT brnNo, brnNm, modlNo, modlNm, pretaxPrice FROM {$g5['shopping_preorder_kr_table']} $sql_where) AS kr LEFT JOIN {$g5['shopping_count_table']} AS cnt ON kr.modlNo = cnt.MODL_C_NO $sql_order");
} else {
    $modelQry = sql_query("SELECT DISTINCT brandNo, brandNm, modelNo, modelNm FROM (SELECT DISTINCT brandNo, brandNm, modelNo, modelNm, carPrice FROM {$g5['shopping_preorder_im_table']} $sql_where) AS im LEFT JOIN {$g5['shopping_count_table']} AS cnt ON im.modelNo = cnt.MODL_C_NO $sql_order");
}
