...
function get_layout_menu($menu_datas)
{
    ...
    $output = '';
    foreach ($menu_datas as $item) {
        $item['act'] = $item['me_code'] == substr($g5['me_code'], 0, strlen($item['me_code'])) ? 'on' : '';

        if (!$item['sub']) {

            $output .= '<li class=" ' . $item['act'] . '"><a href="' . str_replace("#", "javascript:;", $item['me_link']) . '" target="_' . $item['me_target'] . '" class="">' . $item['me_name'] . '</a></li>';
            ...
            $output .= '<li class=" ' . $item['act'] . '"><a href="' . str_replace("#", "javascript:;", $item['me_link']) . '" target="_' . $item['me_target'] . '" class="">' . $item['me_name'] . '</a><div class="header-submenu">';
            ...
            foreach ($item['sub'] as $item2) {
                $item2['act'] = $item2['me_code'] == substr($g5['me_code'], 0, strlen($item2['me_code'])) ? 'on' : '';

                if ($item2['me_id'] == -1)
                    $output .= ''; //$output .= '<div class="dropdown-divider"></div>';
                else
                    $output .= '<a href="' . $item2['me_link'] . '" class="' . $item2['act'] . '">' . $item2['me_name'] . '</a>';
